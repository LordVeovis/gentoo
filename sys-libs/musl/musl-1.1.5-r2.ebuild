...
src_prepare() {
	...
	epatch "${FILESDIR}"/"${PN}"-1.1.5-dynlink-without-dt_runpath.patch

	# add support for non-option arguments extension to getopt
	epatch "${FILESDIR}"/"${PN}"-1.1.5-support-non-option-args.patch

	# add support for abbreviated options in getopt_long
	epatch "${FILESDIR}"/"${PN}"-1.1.5-support-abbreviated-options.patch
