...
CDEPEND="
	...
	net-misc/curl[ssl]
	apache? ( www-servers/apache[apache2_modules_unixd(+)] )"
...
src_prepare() {
	packaging_file="${S}/all/${P}"/build/packaging.rb

	use apache || sed -i -e 's/^\(task :fakeroot.+\):apache2, /\1/' "$packaging_file"
	sed -i -e "s/^\(task :fakeroot.+\):nginx, 'nginx:as_dynamic_module', /\1AGENT_TARGET/" "$packaging_file"
	#sed -i -e '/# C++ support libraries/,+7d' "$packaging_file"
	#sed -i -e '/# Support binaries/,+7d' "$packaging_file"

	epatch -d"${S}/all/${P}" "${FILESDIR}"/${P}-isnan.patch
